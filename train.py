# train.py  -- improved for higher macro-F1 (oversampling, label-smoothing, scheduler, weighted ensemble)
import os
...
USE_LARGE = os.environ.get("USE_LARGE", "0") == "1"        # set 1 to try -large (only if GPU can)
USE_8BIT = os.environ.get("USE_8BIT", "0") == "1"          # set 1 to use bitsandbytes 8-bit
ENABLE_CHECKPOINTING = os.environ.get("ENABLE_CHECKPOINTING", "0") == "1"
USE_FP16 = os.environ.get("USE_FP16", "0") == "1"
USE_FOCAL = os.environ.get("USE_FOCAL", "0") == "1"        # set 1 to use focal loss instead of label-smoothed CE
OVERSAMPLE = os.environ.get("OVERSAMPLE", "1") == "1"      # set 0 to disable oversampling
...
LABEL_SMOOTHING = float(os.environ.get("LABEL_SMOOTHING", 0.1))  # used if USE_FOCAL==False

# bitsandbytes detection
BNB_AVAILABLE = False
if USE_8BIT:
    try:
...
def main():
    ...
    print("CONFIG:", {"MODEL": MODEL_CHECKPOINT, "USE_8BIT": USE_8BIT, "USE_FOCAL": USE_FOCAL, "OVERSAMPLE": OVERSAMPLE, "EPOCHS": EPOCHS, "LR": LR})

    df = preprocess_data(TRAIN_FILE_PATH)
    if df.empty:
        raise RuntimeError("No data found in TRAIN_FILE_PATH")
    ...
    for fold in range(1):
        ...
        # Oversample minority classes (simple deterministic upsampling to max class count)
        if OVERSAMPLE:
            ...
            for label, cnt in counts.items():
                ...
                parts.append(subset)
            train_df_fold = pd.concat(parts, ignore_index=True)
            train_df_fold = train_df_fold.sample(frac=1.0, random_state=SEED).reset_index(drop=True)
        ...
        training_args = TrainingArguments(
            ...
            save_total_limit=1,
            report_to="none",
            bf16=USE_FP16 and False,  # leave bf16 False unless you know environment supports; USE_FP16 toggles fp16 below
            fp16=USE_FP16,
            dataloader_pin_memory=True,
            lr_scheduler_type="cosine",
            label_smoothing_factor=0.0,  # we implement label smoothing in trainer to support class weights
        )
        ...
        fold_weights.append(max(0.0, val_f1))  # use val_f1 as weight (non-negative)
    ...
    # Weighted ensemble by fold val_f1
    weights = np.array(fold_weights, dtype=float)
    if weights.sum() <= 0:
        weights = np.ones(len(all_test_preds), dtype=float) / len(all_test_preds)
    else:
        weights = weights / weights.sum()
    stacked = np.stack(all_test_preds, axis=0)  # shape (n_folds, n_samples, n_classes)
    # compute weighted average across folds
    avg = np.tensordot(weights, stacked, axes=(0,0))  # shape (n_samples, n_classes)
